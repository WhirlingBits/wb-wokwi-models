/*
 * EZO-pH Wokwi Simulation Example (ESP-IDF)
 *
 * Demonstrates communication with the Atlas Scientific EZO-pH
 * circuit using the EZO I2C ASCII protocol.
 *
 * Wiring (diagram.json):
 *   GPIO 4  → SDA
 *   GPIO 5  → SCL
 ...
 *   3. Performs mid-point calibration ("Cal,mid,7.00")
 *   4. Queries probe slope ("Slope,?")
 *   5. Reads pH in a loop ("R")
 */
...
static const char *TAG = "ezo-ph-example";
...
void app_main(void)
{
    ...
    /* Perform mid-point calibration with pH 7.00 */
    ESP_LOGI(TAG, "Performing mid-point calibration...");
    if (ezo_command("Cal,mid,7.00", EZO_DELAY_PH_MS, resp, sizeof(resp)) == ESP_OK) {
        ESP_LOGI(TAG, "Mid-point calibration done");
    }

    /* Query calibration again */
    if (ezo_command("Cal,?", EZO_DELAY_GENERIC_MS, resp, sizeof(resp)) == ESP_OK) {
        ESP_LOGI(TAG, "Calibration after mid: %s", resp);
    }

    /* Query probe slope */
    if (ezo_command("Slope,?", EZO_DELAY_GENERIC_MS, resp, sizeof(resp)) == ESP_OK) {
        ESP_LOGI(TAG, "Slope: %s", resp);
    }

    /* Continuous pH reading loop */
    while (1) {
        memset(resp, 0, sizeof(resp));

        if (ezo_command("R", EZO_DELAY_PH_MS, resp, sizeof(resp)) == ESP_OK) {
            ESP_LOGI(TAG, "pH: %s", resp);
        }
